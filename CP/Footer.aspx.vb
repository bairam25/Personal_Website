...
#Region "Global_Varaibles"
    ...
    Dim FAQTable As String = "Select * from tblFooter where isnull(IsDeleted,0)=0"
...
#Region "Public Functions"
    ...
    Sub SetControlFields()
        Try
            URLTitle = txtTitle.Text.Trim
            URL = txtURL.Text.Trim
            ShowOrder = Val(txtShowOrder.Text)
        Catch ex As Exception
            clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page, ex)
        End Try
    End Sub
...
#Region "Add"
    ...
    Protected Sub Add(ByVal Sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            Enabler(True)
            pf.ClearAll(pnlForm)
            txtShowOrder.Text = Val(DBManager.Getdatatable("select count(*) from tblFooter where isnull(IsDeleted,0)=0 ").Rows(0).Item(0).ToString) + 1
...
#Region "Save"
    ...
    Protected Sub Save(ByVal Sender As Object, ByVal e As System.EventArgs)
        Dim daFooter As New TblFooterFactory
        Dim dtFooter As New TblFooter
        Try
            If cmdSave.CommandArgument.ToLower() = "add" Then
                'Insert Case
                'Fill object tblFooter
                If Not FillDT(dtFooter) Then
                    Return
                End If
                'insert at tblFooter
                If daFooter.Insert(dtFooter) Then
            ...
            Else
                'Update Case
                dtFooter = daFooter.GetAllBy(TblFooter.TblFooterFields.Id, lblFAQID.Text)(0)
                'Fill object of tblFooter
                If Not FillDT(dtFooter) Then
                    Return
                End If
                'Update tblFooter
                If daFooter.Update(dtFooter) Then
    ...
    End Sub
    ''' <summary>
    ''' Fill tblFooter Object
    ''' </summary>
    Function FillDT(ByVal dtFooter As TblFooter) As Boolean
        Try
            '2)Validation
            If Not IsNumeric(txtShowOrder.Text) Then
                ...
                Return False
            End If
            dtFooter.Title = URLTitle
            dtFooter.URL = URL
            dtFooter.ShowOrder = Val(txtShowOrder.Text)
            If cmdSave.CommandArgument.ToLower() = "add" Then
                dtFooter.CreatedDate = DateTime.Now
                dtFooter.CreatedBy = UserId
                dtFooter.Active = False
            End If
            dtFooter.ModifiedDate = DateTime.Now
            dtFooter.ModifiedBy = UserId
            dtFooter.IsDeleted = False
            Return True
        Catch ex As Exception
            clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page, ex)
            Return False
...
#Region "Fill Grid"
            ...
            If dt IsNot Nothing Then
                If dt.Rows.Count > 0 Then
                    ...
                    If sender.id = "txtSearch" Or sender.id = "lbSearchIcon" Then
                        ...
                        dplvFooters.SetPageProperties(0, ddlPager.SelectedValue, True)
                    End If
                    BindListView()
                    dplvFooters.Visible = False
                    If dt.Rows.Count > ddlPager.SelectedValue Then
                        dplvFooters.Visible = True
                    End If
                Else
                    lvFooters.DataSource = Nothing
                    lvFooters.DataBind()
                    dplvFooters.Visible = False
                End If
    ...
    ''' <summary>
    ''' Collect condition string to fill lvFooters
    ''' </summary>
    ...
    ''' <summary>
    ''' Load faq data from [tblFooters] table into the Listview.
    ''' </summary>
    Private Sub BindListView()
        Try
            If ViewState("dtFooterTable") IsNot Nothing Then
                ' Get the DataTable from ViewState.
                Dim dtFooterTable As DataTable = DirectCast(ViewState("dtFooterTable"), DataTable)

                ' Convert the DataTable to DataView.
                Dim dv As New DataView(dtFooterTable)
                ...
                lvFooters.DataSource = dv
                lvFooters.DataBind()
                If dtFooterTable.Rows.Count > 0 Then
                    dplvFooters.DataBind()
                End If
            End If
        Catch ex As Exception
            clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page, ex)
    ...
    Protected Sub OnPagePropertiesChanging(sender As Object, e As PagePropertiesChangingEventArgs)
        Try
            dplvFooters.SetPageProperties(e.StartRowIndex, e.MaximumRows, False)
            FillGrid(sender, New EventArgs)
        Catch ex As Exception
            clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page, ex)
        End Try
    ...
    Protected Sub lv_Sorting(sender As Object, e As ListViewSortEventArgs)
        Try
            ...
            If strSortExpression(1) = "ASC" Then
                CType(lvFooters.FindControl(e.SortExpression), HtmlTableCell).Attributes.Add("class", "faDown")
            Else
                CType(lvFooters.FindControl(e.SortExpression), HtmlTableCell).Attributes.Add("class", "faUp")
            End If
    ...
    Sub ResetArrows()
        Try
            Dim i As Integer = 0
            While i < lvFooters.Items.Count
                CType(lvFooters.FindControl("ShowOrder"), HtmlTableCell).Attributes.Add("class", "upnDownArrow")
                CType(lvFooters.FindControl("Title"), HtmlTableCell).Attributes.Add("class", "upnDownArrow")
                'CType(lvFooters.FindControl("Description"), HtmlTableCell).Attributes.Add("class", "upnDownArrow")
                'CType(lvFooters.FindControl("Answer"), HtmlTableCell).Attributes.Add("class", "upnDownArrow")
                i += 1
            End While
    ...
    Sub UpdateActive(sender As Object, e As EventArgs)
        Try
            ...
            End If
            Updated = DBManager.ExcuteQuery("Update tblFooter set Active ='" + chk.Checked.ToString + "',ModifiedDate=getdate(),ModifiedBy='" + UserId + "' where Id='" + ItemId + "' ")
            If Updated = 1 Then
                clsLogs.AddSystemLogs(StatusName, "tblFooter", ItemId)
...
#Region "Delete"
    ...
    Protected Sub Delete(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            If DBManager.ExcuteQuery("Update tblFooter set IsDeleted=1,DeletedDate=getdate() where id= '" & Id & "'") = 1 Then
                FillGrid(Sender, e)
...
#End Region
#Region "Permissions"
    Private Sub ListView_DataBound(sender As Object, e As EventArgs) Handles lvFooters.DataBound
        Try
            Permissions.CheckPermisions(lvFooters, lbNew, txtSearch, lbSearchIcon, Me.Page, UserId)
        Catch ex As Exception
            clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page, ex)
        End Try
    End Sub
