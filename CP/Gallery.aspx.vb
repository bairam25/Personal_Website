...
#Region "Add"
    ...
    Protected Sub Add(ByVal Sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            txtShowOrder.Text = Val(DBManager.Getdatatable("select count(*) from tblAlbums where isnull(IsDeleted,0)=0 ").Rows(0).Item(0).ToString) + 1
        Catch ex As Exception
            clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page, ex)
        End Try
    End Sub
...
#Region "Save"
    ...
    Protected Sub Save(ByVal Sender As Object, ByVal e As System.EventArgs)
        Dim daAlbum As New TblAlbumsFactory
        Dim dtAlbum As New TblAlbums
        Try
            ...
            Else
                'Update Case
                dtAlbum = daAlbum.GetAllBy(TblAlbums.TblAlbumsFields.Id, lblAlbumId.Text)(0)
    ...
    ''' </summary>
    Function FillDT(ByVal dtAlbum As TblAlbums) As Boolean
        Try
            '1)Validate 
    ...
    ''' <summary>
    ''' Fill TblMedia from controls in the panel photos.
    ''' </summary>
        ...
        Try
            Select Case Operation.ToLower
                ...
                Case "update"
                    ...
                    ExecuteQuery.ExecuteAlCommands(_sqltrans, _sqlconn, New SqlCommand("delete from TblMedia where SourceId='" & AlbumId & "' and Type='A'"))
                    'daImgs.DeleteTrans(TblMedia.TblMediaFields.SourceId, AlbumId, _sqlconn, _sqltrans)
            End Select
            'loop to get images details from images gridview
            For Each gvRow As GridViewRow In gvItemsImgs.Rows
                dtImgs.SourceId = AlbumId
                dtImgs.Type = "A"
                dtImgs.MediaPath = CType(gvRow.FindControl("lblImg"), System.Web.UI.WebControls.Image).ImageUrl
                dtImgs.MediaThumbPath = dtImgs.MediaPath
                ...
                dtImgs.MediaType = IIf(dtImgs.MediaPath.ToString.Split(".").Last.ToLower = "mp4" OrElse dtImgs.MediaPath.ToString.Split(".").Last.ToLower = "wmv" OrElse dtImgs.MediaPath.ToString.Split(".").Last.ToLower = "webm", "V", "I")
                If dtImgs.MediaType = "I" Then
...
#Region "Delete"
    ...
    Protected Sub Delete(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            If DBManager.ExcuteQuery("update tblAlbums SET Isdeleted = 'True',DeletedDate=GETDATE() where Id= '" & AlbumId & "';Update tblMedia SET Isdeleted = 'True',DeletedDate=GETDATE() where SourceId= '" & AlbumId & "' and Type='A'") = 1 Then
                FillGrid(Sender, e)
    ...
    Protected Sub DeletePhoto(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            If ExecuteQuery.ExecuteAlCommands(_sqltrans, _sqlconn, New SqlCommand("update tblMedia SET Isdeleted = 'True',DeletedDate=GETDATE() where Id= '" & MediaId & "' and Type='A' and SourceId='" & AlbumId & "';update tblAlbums SET MediaCount=MediaCount-1 where Id= '" & AlbumId & "'")) Then
                ...
                If Main Then
                    ...
                    If Not ExecuteQuery.ExecuteAlCommands(_sqltrans, _sqlconn, New SqlCommand("update tblMedia SET Main=1 where Id= '" & FirstPhotoId & "' and SourceId='" & AlbumId & "' and Type='A'")) Then
                        _sqltrans.Rollback()
                        clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page)
                    End If
                End If
                ...
                For Each gvRow As ListViewItem In lvImages.Items
                    ...
                    If Not ExecuteQuery.ExecuteAlCommands(_sqltrans, _sqlconn, New SqlCommand("update tblMedia SET ShowOrder = " & serialNo & " where Id= '" & id & "' and SourceId='" & AlbumId & "' and Type='A'")) Then
                        _sqltrans.Rollback()
                        clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page)
                    End If
                Next
    ...
    Private Sub BindAlbumMedia(ByVal AlbumId As String, ByRef _sqlconn As SqlConnection, ByRef _sqltrans As SqlTransaction)
        Try
            Dim dtNewsImgs As DataTable = ExecuteQuery.ExecuteQueryAndReturnDataTable("select Id,SourceId,ShowOrder,Main,MediaPath,MediaThumbPath from tblMedia where isnull(Isdeleted,0)=0 and SourceId='" + AlbumId + "' and Type='A'", _sqlconn, _sqltrans)
...
#Region "Fill Grid"
    ...
    Sub UpdateActive(sender As Object, e As EventArgs)
        Try
            ...
            End If
            Updated = DBManager.ExcuteQuery("Update TblAlbums set Active ='" + chk.Checked.ToString + "',ModifiedDate=getdate(),ModifiedBy='" + UserID + "' where Id='" + ItemId + "' ")
            If Updated = 1 Then
                clsLogs.AddSystemLogs(StatusName, "TblAlbums", ItemId)
...
#Region "Photo Upload"
    ...
    Sub FillPhotos(ByVal AlbumId As String)
        Try
            Dim dtItemImgs As DataTable = DBManager.Getdatatable("select * from tblMedia where isnull(Isdeleted,0)=0 and  SourceId='" & AlbumId & "' and Type='A'")
    ...
    Sub BindMainImg()
        Try
            ...
            dtItems = DBManager.Getdatatable("select MediaPath from tblMedia where Main='1' and SourceId='" + lblAlbumId.Text + "'")
    ...
    Protected Sub ViewPhotos(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            Dim dtNewsImgs As DataTable = DBManager.Getdatatable("select Id,SourceId,ShowOrder,Main,MediaPath,MediaThumbPath from tblMedia where isnull(Isdeleted,0)=0 and SourceId='" + AlbumId + "' and Type='A'")
            lvImages.DataSource = dtNewsImgs
            lvImages.DataBind()
            mpPopupImgs.Show()
...
#Region "Permissions"
    Private Sub ListView_DataBound(sender As Object, e As EventArgs) Handles lvGallery.DataBound
        Try
            Permissions.CheckPermisions(lvGallery, lbNew, txtSearch, lbSearchIcon, Me.Page, UserID)
        Catch ex As Exception
            clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page, ex)
        End Try
    End Sub
